...
class ParchmentsGetResponse200Normalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize($data, $class, $format = null, array $context = array())
    {
        ...
        if (property_exists($data, 'hydra:view')) {
            $object->setHydra:view($this->denormalizer->denormalize($data->{'hydra:view'}, 'ApiPlatform\\Demo\\Model\\ParchmentsGetResponse200HydraView', 'json', $context));
        }
        if (property_exists($data, 'hydra:search')) {
            $object->setHydra:search($this->denormalizer->denormalize($data->{'hydra:search'}, 'ApiPlatform\\Demo\\Model\\ParchmentsGetResponse200HydraSearch', 'json', $context));
        }
        return $object;
    }
