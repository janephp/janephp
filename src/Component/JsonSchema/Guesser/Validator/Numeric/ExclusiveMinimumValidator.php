...
class ExclusiveMinimumValidator implements ValidatorInterface
{
    ...
    {
        return $this->checkObject($object) && (\is_array($object->type ?? null) ? (\in_array('integer', $object->type ?? null) || \in_array('number', $object->type ?? null)) : ('integer' === ($object->type ?? null) || 'number' === ($object->type ?? null))) && is_numeric($object->exclusiveMinimum ?? null);
    }
    ...
    public function guess($object, string $name, $guess): void
    {
        $guess->addValidatorGuess(new ValidatorGuess(GreaterThan::class, [
            'value' => ($object->exclusiveMinimum ?? null),
        ]));
    }
}
