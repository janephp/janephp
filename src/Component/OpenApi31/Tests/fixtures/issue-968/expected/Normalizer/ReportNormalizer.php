...
class ReportNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('period', $data) && $data['period'] !== null) {
            $object->setPeriod($this->denormalizer->denormalize($data['period'], \Jane\Component\OpenApi31\Tests\Expected\Issue968\Model\ReportPeriod::class, 'json', $context));
            unset($data['period']);
        }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('period') && null !== $data->getPeriod()) {
            $dataArray['period'] = $data->getPeriod() === null ? null : new \Jane\Component\OpenApi31\Tests\Expected\Issue968\Runtime\JsonObject($this->normalizer->normalize($data->getPeriod(), 'json', $context));
        }
