    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if (array_key_exists('what', get_object_vars($data)) && null !== ($data->what ?? null)) {
            ...
            if (is_object($data->what)) {
                ...
                $value = \is_iterable($normalized) ? new \Jane\Component\OpenApi31\Tests\DiscriminatorExpected\Runtime\JsonObject($normalized) : $normalized;
            } elseif (is_object($data->what)) {
                $normalized_1 = $this->normalizer->normalize($data->what, 'json', $context);
                $value = \is_iterable($normalized_1) ? new \Jane\Component\OpenApi31\Tests\DiscriminatorExpected\Runtime\JsonObject($normalized_1) : $normalized_1;
            }
