...
class ZoneCustomizedTimeZoneNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('start', $data)) {
            $object->setStart($this->denormalizer->denormalize($data['start'], \Jane\Component\OpenApi3\Tests\Expected\Issue770\Model\ZoneDaylightSavingTime::class, 'json', $context));
        }
        if (\array_key_exists('end', $data)) {
            $object->setEnd($this->denormalizer->denormalize($data['end'], \Jane\Component\OpenApi3\Tests\Expected\Issue770\Model\ZoneDaylightSavingTime::class, 'json', $context));
        }
        return $object;
    }
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('start') && null !== $data->getStart()) {
            $dataArray['start'] = $data->getStart() === null ? null : new \Jane\Component\OpenApi3\Tests\Expected\Issue770\Runtime\JsonObject($this->normalizer->normalize($data->getStart(), 'json', $context));
        }
        if ($data->isInitialized('end') && null !== $data->getEnd()) {
            $dataArray['end'] = $data->getEnd() === null ? null : new \Jane\Component\OpenApi3\Tests\Expected\Issue770\Runtime\JsonObject($this->normalizer->normalize($data->getEnd(), 'json', $context));
        }
        return $dataArray;
    }
