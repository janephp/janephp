...
class IdentityQueryCriteriaOptionsNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('localUser_auditTime', $data)) {
            $object->setLocalUserAuditTime($this->denormalizer->denormalize($data['localUser_auditTime'], \Jane\Component\OpenApi3\Tests\Expected\Issue770\Model\IdentityQueryCriteriaOptionsLocalUserAuditTime::class, 'json', $context));
        }
        ...
        if (\array_key_exists('guestPass_expiration', $data)) {
            $object->setGuestPassExpiration($this->denormalizer->denormalize($data['guestPass_expiration'], \Jane\Component\OpenApi3\Tests\Expected\Issue770\Model\IdentityQueryCriteriaOptionsGuestPassExpiration::class, 'json', $context));
        }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('localUserAuditTime') && null !== $data->getLocalUserAuditTime()) {
            $dataArray['localUser_auditTime'] = $data->getLocalUserAuditTime() === null ? null : new \Jane\Component\OpenApi3\Tests\Expected\Issue770\Runtime\JsonObject($this->normalizer->normalize($data->getLocalUserAuditTime(), 'json', $context));
        }
        ...
        if ($data->isInitialized('guestPassExpiration') && null !== $data->getGuestPassExpiration()) {
            $dataArray['guestPass_expiration'] = $data->getGuestPassExpiration() === null ? null : new \Jane\Component\OpenApi3\Tests\Expected\Issue770\Runtime\JsonObject($this->normalizer->normalize($data->getGuestPassExpiration(), 'json', $context));
        }
