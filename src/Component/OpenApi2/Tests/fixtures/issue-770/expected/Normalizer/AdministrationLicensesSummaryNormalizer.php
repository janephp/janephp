...
class AdministrationLicensesSummaryNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if (array_key_exists('licenseTypeDescription', get_object_vars($data)) && null !== ($data->licenseTypeDescription ?? null)) {
            $dataArray['licenseTypeDescription'] = $data->licenseTypeDescription;
        }
        if (array_key_exists('capacityControlLicenseCount', get_object_vars($data)) && null !== ($data->capacityControlLicenseCount ?? null)) {
            $normalized = $this->normalizer->normalize($data->capacityControlLicenseCount, 'json', $context);
            $dataArray['capacityControlLicenseCount'] = \is_iterable($normalized) ? new \Jane\Component\OpenApi3\Tests\Expected\Issue770\Runtime\JsonObject($normalized) : $normalized;
        }
        return $dataArray;
    }
