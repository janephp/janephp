...
class DpskWlanExternalDpskNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('encryption', $data)) {
            $object->encryption = $this->denormalizer->denormalize($data['encryption'], \Jane\Component\OpenApi3\Tests\Expected\Issue770\Model\DpskWlanExternalDpskEncryption::class, 'json', $context);
        }
        if (\array_key_exists('authService', $data)) {
            $object->authService = $this->denormalizer->denormalize($data['authService'], \Jane\Component\OpenApi3\Tests\Expected\Issue770\Model\DpskWlanExternalDpskAuthService::class, 'json', $context);
        }
        return $object;
    }
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if (array_key_exists('authService', get_object_vars($data)) && null !== ($data->authService ?? null)) {
            $normalized_1 = $this->normalizer->normalize($data->authService, 'json', $context);
            $dataArray['authService'] = \is_iterable($normalized_1) ? new \Jane\Component\OpenApi3\Tests\Expected\Issue770\Runtime\JsonObject($normalized_1) : $normalized_1;
        }
        return $dataArray;
    }
