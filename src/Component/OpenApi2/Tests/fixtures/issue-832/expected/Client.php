...
class Client extends \Jane\Component\OpenApi2\Tests\Issue832\Runtime\Client\Client
{
    ...
    {
        return $this->executeEndpoint(new \Jane\Component\OpenApi2\Tests\Issue832\Endpoint\Ping(), $fetch);
    }
    /**
     * @param string $fetch Fetch mode to use (can be OBJECT or RESPONSE)
     *
     * @return ($fetch is 'object' ? null : \Psr\Http\Message\ResponseInterface)
     */
    public function prefixStatuscheck(string $fetch = self::FETCH_OBJECT)
    {
        return $this->executeEndpoint(new \Jane\Component\OpenApi2\Tests\Issue832\Endpoint\PrefixStatuscheck(), $fetch);
    }
