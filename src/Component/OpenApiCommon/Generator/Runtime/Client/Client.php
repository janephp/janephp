...
abstract class Client
{
    ...
    public function executeEndpoint(Endpoint $endpoint): mixed
    {
        return match ($endpoint->getFetchMode()) {
            ...
            FetchMode::Preload->value => new Result(
                $this->processEndpoint($endpoint),
                fn (ResponseInterface $response): mixed => $endpoint->parseResponse($response, $this->serializer)
            ),
            default => new Result(
                fn (): ResponseInterface => $this->processEndpoint($endpoint),
                fn (ResponseInterface $response): mixed => $endpoint->parseResponse($response, $this->serializer)
            ),
        };
    }
