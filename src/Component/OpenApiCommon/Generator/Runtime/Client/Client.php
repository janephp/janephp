...
abstract class Client
{
    ...
    private function executeDeferredEndpoint(Endpoint $endpoint): mixed
    {
        ...
        $response = FetchMode::Preload->value === $endpoint->getFetchMode()
            ? $this->processEndpoint($endpoint)
            : null;

        $proxy = GhostFactory::create(
            $targetClass,
            fn (): mixed => $endpoint->parseResponse($response ?? $this->processEndpoint($endpoint), $this->serializer)
        );
