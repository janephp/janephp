...
class UndeclaredStatusResponseTest extends TestCase
{
    public function testAnUndeclaredStatusThrowsByDefault(): void
    {
        ...
        try {
            ...
            self::assertStringContainsString(
                "throw new \\Jane\\Component\\OpenApi3\\Tests\\UndeclaredStatusExpected\\Exception\\BadResponseException(\$status, \$body, \$response);\n    }",
                $endpointContent
            );
            self::assertStringContainsString('@throws \Jane\Component\OpenApi3\Tests\UndeclaredStatusExpected\Exception\BadResponseException', $endpointContent);
            self::assertStringContainsString("@return \\Jane\\Component\\OpenApi3\\Tests\\UndeclaredStatusExpected\\Model\\Pet\n", $endpointContent);
    ...
    public function testAnUndeclaredStatusReturnsAnExplicitNullWhenThrowingIsSwitchedOff(): void
    {
        ...
        try {
            ...
            self::assertStringContainsString("@return null|\\Jane\\Component\\OpenApi3\\Tests\\UndeclaredStatusExpected\\Model\\Pet\n", $endpointContent);
    ...
    /**
     * When no declared response yields a value (here: a binary 200 the
     * generator does not deserialize), the docblock falls back to the
     * `mixed` of the abstract method instead of an empty `@return`.
     */
    public function testTheReturnTypeFallsBackToMixedWhenNoResponseYieldsAValue(): void
    {
        [$fixtureDirectory, $generatedDirectory] = $this->generateClient([], false, 'image/png');

        try {
            $endpointContent = file_get_contents($generatedDirectory . '/Endpoint/GetPet.php');
            self::assertIsString($endpointContent);
            self::assertStringContainsString("     * @return mixed\n     */\n    protected function transformResponseBody(", $endpointContent);
            self::assertStringContainsString("throw new \\Jane\\Component\\OpenApi3\\Tests\\UndeclaredStatusExpected\\Exception\\BadResponseException(\$status, \$body, \$response);\n    }", $endpointContent);
        } finally {
            $this->removeDirectory($fixtureDirectory);
        }
    }
    ...
    /**
     * A `default` response without content already closes the chain with an
     * unconditional return: nothing is appended after it.
     */
    public function testNothingIsAppendedAfterAContentLessDefaultResponse(): void
    {
     ...
     */
    private function generateClient(array $options, bool $withDefaultResponse = false, string $contentType = 'application/json'): array
    {
        $fixtureDirectory = sys_get_temp_dir() . '/jane-openapi3-undeclared-status-' . bin2hex(random_bytes(8));
        ...
        $responses = [
            '200' => [
                ...
                'content' => [
                    $contentType => [
                        'schema' => 'application/json' === $contentType ? ['$ref' => '#/components/schemas/Pet'] : ['type' => 'string', 'format' => 'binary'],
                    ],
                ],
            ],
        ...
        file_put_contents($configFile, '<?php' . "\n\n" . 'return ' . var_export($options + [
            ...
            'namespace' => 'Jane\Component\OpenApi3\Tests\UndeclaredStatusExpected',
