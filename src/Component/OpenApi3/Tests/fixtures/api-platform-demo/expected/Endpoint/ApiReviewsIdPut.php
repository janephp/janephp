...
class ApiReviewsIdPut extends \ApiPlatform\Demo\Runtime\Client\BaseEndpoint implements \ApiPlatform\Demo\Runtime\Client\Endpoint
{
    ...
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        ...
        if (400 === $status) {
            throw new \ApiPlatform\Demo\Exception\ApiReviewsIdPutBadRequestException();
        }
        if (422 === $status) {
            throw new \ApiPlatform\Demo\Exception\ApiReviewsIdPutUnprocessableEntityException();
        }
        if (404 === $status) {
            throw new \ApiPlatform\Demo\Exception\ApiReviewsIdPutNotFoundException();
        }
    }
