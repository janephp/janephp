...
class SchemaNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('objectProperty', $data)) {
            $object->setObjectProperty($this->denormalizer->denormalize($data['objectProperty'], \Jane\Component\OpenApi3\Tests\Expected\ModelInResponse\Model\SchemaObjectProperty::class, 'json', $context));
            unset($data['objectProperty']);
        }
        if (\array_key_exists('objectRefProperty', $data)) {
            $object->setObjectRefProperty($this->denormalizer->denormalize($data['objectRefProperty'], \Jane\Component\OpenApi3\Tests\Expected\ModelInResponse\Model\Schema::class, 'json', $context));
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('objectProperty') && null !== $data->getObjectProperty()) {
            $dataArray['objectProperty'] = $data->getObjectProperty() === null ? null : new \Jane\Component\OpenApi3\Tests\Expected\ModelInResponse\Runtime\JsonObject($this->normalizer->normalize($data->getObjectProperty(), 'json', $context));
        }
        if ($data->isInitialized('objectRefProperty') && null !== $data->getObjectRefProperty()) {
            $dataArray['objectRefProperty'] = $data->getObjectRefProperty() === null ? null : new \Jane\Component\OpenApi3\Tests\Expected\ModelInResponse\Runtime\JsonObject($this->normalizer->normalize($data->getObjectRefProperty(), 'json', $context));
        }
