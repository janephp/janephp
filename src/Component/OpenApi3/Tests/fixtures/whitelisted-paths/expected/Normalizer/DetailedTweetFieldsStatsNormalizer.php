...
class DetailedTweetFieldsStatsNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return $type === \Jane\Component\OpenApi3\Tests\Expected\WhitelistedPaths\Model\DetailedTweetFieldsStats::class;
    }
    public function supportsNormalization(mixed $data, ?string $format = null, array $context = []): bool
    {
        return is_object($data) && get_class($data) === \Jane\Component\OpenApi3\Tests\Expected\WhitelistedPaths\Model\DetailedTweetFieldsStats::class;
    }
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        $object = new \Jane\Component\OpenApi3\Tests\Expected\WhitelistedPaths\Model\DetailedTweetFieldsStats();
        ...
        if (\array_key_exists('like_count', $data)) {
            $object->likeCount = $data['like_count'];
            unset($data['like_count']);
        }
        if (\array_key_exists('quote_count', $data)) {
            $object->quoteCount = $data['quote_count'];
    ...
    {
        $dataArray = [];
        $dataArray['retweet_count'] = $data->retweetCount;
        $dataArray['reply_count'] = $data->replyCount;
        $dataArray['like_count'] = $data->likeCount;
        if (array_key_exists('quoteCount', get_object_vars($data)) && null !== ($data->quoteCount ?? null)) {
            $dataArray['quote_count'] = $data->quoteCount;
        }
    ...
    {
        return [\Jane\Component\OpenApi3\Tests\Expected\WhitelistedPaths\Model\DetailedTweetFieldsStats::class => false];
    }
}