...
class FilePostBodyNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if (array_key_exists('item', get_object_vars($data)) && null !== ($data->item ?? null)) {
            $normalized = $this->normalizer->normalize($data->item, 'json', $context);
            $dataArray['item'] = \is_iterable($normalized) ? new \Jane\Component\OpenApi3\Tests\Expected\MultipartNestedObject\Runtime\JsonObject($normalized) : $normalized;
        }
