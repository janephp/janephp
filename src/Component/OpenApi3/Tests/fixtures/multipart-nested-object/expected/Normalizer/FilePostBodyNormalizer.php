...
class FilePostBodyNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('item', $data)) {
            $object->setItem($this->denormalizer->denormalize($data['item'], \Jane\Component\OpenApi3\Tests\Expected\MultipartNestedObject\Model\FilePostBodyItem::class, 'json', $context));
            unset($data['item']);
        }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('item') && null !== $data->getItem()) {
            $dataArray['item'] = $data->getItem() === null ? null : new \Jane\Component\OpenApi3\Tests\Expected\MultipartNestedObject\Runtime\JsonObject($this->normalizer->normalize($data->getItem(), 'json', $context));
        }
