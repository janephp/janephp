...
class ReposOwnerRepoBranchesBranchProtectionRequiredStatusChecksPatchBodyNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        $dataArray = [];
        if ($data->isInitialized('strict') && null !== $data->getStrict()) {
            $dataArray['strict'] = $data->getStrict();
        }
        if ($data->isInitialized('contexts') && null !== $data->getContexts()) {
            $values = [];
        ...
        if (!($context['skip_validation'] ?? false)) {
            $this->validate($dataArray, new \Github\Validator\ReposOwnerRepoBranchesBranchProtectionRequiredStatusChecksPatchBodyConstraint());
        }
        return $dataArray;
    }
