...
class BazNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('sub', $data)) {
            $object->setSub($this->denormalizer->denormalize($data['sub'], \Jane\Component\OpenApi3\Tests\Expected\WhitelistedPathsCircularReference\Model\SubBaz::class, 'json', $context));
            unset($data['sub']);
        }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('sub') && null !== $data->getSub()) {
            $dataArray['sub'] = $data->getSub() === null ? null : new \Jane\Component\OpenApi3\Tests\Expected\WhitelistedPathsCircularReference\Runtime\JsonObject($this->normalizer->normalize($data->getSub(), 'json', $context));
        }
