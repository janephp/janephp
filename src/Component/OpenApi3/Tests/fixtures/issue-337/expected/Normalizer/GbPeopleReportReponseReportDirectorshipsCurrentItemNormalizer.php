...
class GbPeopleReportReponseReportDirectorshipsCurrentItemNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if (array_key_exists('creditScore', get_object_vars($data)) && null !== ($data->creditScore ?? null)) {
            $normalized_1 = $this->normalizer->normalize($data->creditScore, 'json', $context);
            $dataArray['creditScore'] = \is_iterable($normalized_1) ? new \CreditSafe\API\Runtime\JsonObject($normalized_1) : $normalized_1;
        }
        ...
        if (array_key_exists('latestTurnoverFigure', get_object_vars($data)) && null !== ($data->latestTurnoverFigure ?? null)) {
            $normalized_3 = $this->normalizer->normalize($data->latestTurnoverFigure, 'json', $context);
            $dataArray['latestTurnoverFigure'] = \is_iterable($normalized_3) ? new \CreditSafe\API\Runtime\JsonObject($normalized_3) : $normalized_3;
        }
        if (array_key_exists('netWorth', get_object_vars($data)) && null !== ($data->netWorth ?? null)) {
            $normalized_4 = $this->normalizer->normalize($data->netWorth, 'json', $context);
            $dataArray['netWorth'] = \is_iterable($normalized_4) ? new \CreditSafe\API\Runtime\JsonObject($normalized_4) : $normalized_4;
        }
        if (array_key_exists('legalAmount', get_object_vars($data)) && null !== ($data->legalAmount ?? null)) {
            $normalized_5 = $this->normalizer->normalize($data->legalAmount, 'json', $context);
            $dataArray['legalAmount'] = \is_iterable($normalized_5) ? new \CreditSafe\API\Runtime\JsonObject($normalized_5) : $normalized_5;
        }
