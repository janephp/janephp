...
class RootNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function denormalize(mixed $data, string $type, ?string $format = null, array $context = []): mixed
    {
        ...
        if (\array_key_exists('foo', $data) && $data['foo'] !== null) {
            $object->setFoo($this->denormalizer->denormalize($data['foo'], \Jane\Component\OpenApi3\Tests\Expected\AllOfNullableReferenceProperty\Model\RootFoo::class, 'json', $context));
            unset($data['foo']);
        }
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        if ($data->isInitialized('foo') && null !== $data->getFoo()) {
            $dataArray['foo'] = $data->getFoo() === null ? null : new \Jane\Component\OpenApi3\Tests\Expected\AllOfNullableReferenceProperty\Runtime\JsonObject($this->normalizer->normalize($data->getFoo(), 'json', $context));
        }
