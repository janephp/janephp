...
class Client extends \Jane\Component\OpenApi3\Tests\Expected\Runtime\Client\Client
{
     ...
     *
     * @return ($fetch is 'object' ? null|\Jane\Component\OpenApi3\Tests\Expected\Model\ResponseCommon : \Psr\Http\Message\ResponseInterface)
     */
    public function testReferenceResponse(string $fetch = self::FETCH_OBJECT)
    {
        return $this->executeEndpoint(new \Jane\Component\OpenApi3\Tests\Expected\Endpoint\TestReferenceResponse(), $fetch);
    }
    /**
     * @param string $fetch Fetch mode to use (can be OBJECT or RESPONSE)
     *
    ...
    {
        return $this->executeEndpoint(new \Jane\Component\OpenApi3\Tests\Expected\Endpoint\TestRefArray(), $fetch);
    }
