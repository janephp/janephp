...
class GeoDistanceAggregatorNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    public function normalize(mixed $data, ?string $format = null, array $context = []): array|string|int|float|bool|\ArrayObject|null
    {
        ...
        $value_4 = $data->location ?? null;
        if (is_object($data->location ?? null)) {
            $value_4 = ($data->location ?? null) === null ? null : new \PicturePark\API\Runtime\JsonObject($this->normalizer->normalize($data->location ?? null, 'json', $context));
        }
        $dataArray['location'] = $value_4;
        $values_2 = [];
        foreach ($data->ranges ?? null as $value_5) {
