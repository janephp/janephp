...
class Client extends \Jane\Component\OpenApi3\Tests\Issue823\Runtime\Client\Client
{
     ...
     */
    public function getApiUser(string $fetch = self::FETCH_OBJECT)
    {
        return $this->executeEndpoint(new \Jane\Component\OpenApi3\Tests\Issue823\Endpoint\GetApiUser(), $fetch);
    }
    /**
     * @param string $fetch Fetch mode to use (can be OBJECT or RESPONSE)
     *
     * @return ($fetch is 'object' ? null : \Psr\Http\Message\ResponseInterface)
     */
    public function getApiUser2(string $fetch = self::FETCH_OBJECT)
    {
        return $this->executeEndpoint(new \Jane\Component\OpenApi3\Tests\Issue823\Endpoint\GetApiUser2(), $fetch);
    }
