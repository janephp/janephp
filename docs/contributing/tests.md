...
1. **Generation exits cleanly.** A spec refused with a clean Jane error (pre-generation validation, unsupported
   ...
   behaviour and not a failure; a crash (any other exception, a PHP fatal, a timeout) is. A raw PHP error a
   generator raises reaches the command wrapped in a `GenerationFailedException`, itself a Jane exception; the job
   classifies it as a crash all the same.
