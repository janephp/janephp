# ADR 0008: Generated code is statically analysed with Mago
...
8–9), `invalid-iterator` / `invalid-member-selector` (measured: every finding
...
1. **Generated code is analysed with Mago**, configured in
   ...
   hold the corpus.
2. **Strictness-preference codes are switched off, not baselined.** The 16
   ...
   redesign request, not a bug report. Baselining them would bury the ~24 000
   genuine findings under 150 000 entries nobody intends to burn down. Each
...
- The baseline is a work list, not a resting place: its entries collapse into
  ...
  single one — HTTP exception classes assigning a possibly-null
  `ResponseInterface` to a non-nullable property — accounts for 4 609
  findings).
