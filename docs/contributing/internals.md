...
Then, each generator will analyze the metadata and create PHP code by using the
[PHP Parser Library](https://github.com/nikic/PHP-Parser). Using the library improves the flexibility to create
complex code, as using a template generator solution.
...
When the code is ready, the `Context` is read to generate PHP files and optionally format it with
[PHP CS Fixer](http://cs.sensiolabs.org/) if available.
