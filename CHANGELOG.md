...
- [JsonSchema] [OpenApi] Console commands renamed to `jane:generate` / `jane:dump-config` (JsonSchema) and `jane:open-api:generate` / `jane:open-api:dump-config` (OpenAPI); the previous bare `generate` / `dump-config` names remain available as backward-compatibility aliases, and `Application::VERSION` now derives from Composer's installed versions instead of a stale constant
