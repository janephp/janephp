...
## [Unreleased]
### Added
- [Tests] Nightly, non-blocking **corpus smoke job** (`castor qa:corpus`, `.github/workflows/corpus-smoke.yml`): generates a client for each vendor-published real-world spec pinned in `corpus/specs.json` (Kubernetes, Docker, Slack, Netlify, GitHub 3.0 / 3.1, Stripe, Plaid, Box, Twilio, Discord, OpenAI) and asserts that generation does not crash and the output parses, with a report-only Mago summary — no committed output or baselines ([#1041](https://github.com/janephp/janephp/issues/1041) point 3). The fixture tests' php-parser syntax gate moved into the reusable `PhpSyntaxGate`
...
### Changed
- **BC-breaking** [OpenApi] The constructor of the generated runtime `Client` (and therefore of the generated `Client` extending it) is now `final`: `Client::create()` ends in `new static($httpClient, $serializer)`, which is only safe when no subclass can change the constructor's signature. Subclasses still add or override operation methods; construction is wired through `create()` (`$additionalPlugins`, `$additionalNormalizers`) or by calling the constructor with your own `HttpClientInterface` and `SerializerInterface` ([#1066](https://github.com/janephp/janephp/issues/1066), `unsafe-instantiation`)
...
### Fixed
- [JsonSchema] [OpenApi] Generated denormalizers for `format: date-time` (and inline oneOf/anyOf union branches) no longer reject valid RFC 3339 values that the strict `\DateTimeInterface::RFC3339` format cannot parse: a `Z` designator instead of a numeric offset, and optional fractional seconds (`2026-08-31T15:39:33.601Z` threw `InvalidDateException`). When the input format is the default, the strict parse failure is retried with a bare `new \DateTime(...)` gated by a strict RFC 3339 shape regex, and the union condition routes matching values to the date branch so relative date strings (`tomorrow`) can never reach the fallback; values outside the shape keep the same clean `InvalidDateException`, and a custom `date-input-format` keeps the fully strict behavior. The validation generator emits a Symfony `Regex` constraint with the same RFC 3339 shape for default-format `date-time` properties instead of the strict `DateTime(format)` constraint, so validators and denormalizers accept and reject the same values (well-shaped but impossible values such as `2026-02-30T25:61:61Z` are rolled over by `new \DateTime` without exception on both sides). (ported from 7.x, [GH#1102](https://github.com/janephp/janephp/issues/1102), shipped in v7.14.4)
- [OpenApi] The runtime `BaseEndpoint` no longer percent-encodes nested query parameter keys twice. `encodeArrayValue()`, `encodeFormStyle()` and `flattenBracketPairs()` pre-encoded each sub-key before appending it to the bracketed key, which `encodeStringValue()` / `encodeIntValue()` then encoded again: a sub-key containing a reserved character was emitted as `queryParam%5Bunit%253Amm%5D=width` instead of `queryParam%5Bunit%3Amm%5D=width`, so after the server's own URL-decoding the sub-key arrived as the literal string `unit%3Amm` rather than `unit:mm`. Sub-keys are now appended raw and encoded once, by the single `rawurlencode()` of the assembled key, matching `http_build_query(..., PHP_QUERY_RFC3986)`. Regressed in 7.14.0 for `style` / `explode` / `deepObject` parameters and in 7.10.4 ([GH#907](https://github.com/janephp/janephp/pull/907), [GH#908](https://github.com/janephp/janephp/pull/908)) for bracketed arrays; keys made only of unreserved characters are unaffected (ported from 7.x, [#1077](https://github.com/janephp/janephp/pull/1077), shipped in v7.14.3)
- [JsonSchema] [OpenApi] `whitelisted-paths` no longer prunes models that are referenced only through a `oneOf` / `anyOf` union, a nested array, a map, or `additionalProperties` / `patternProperties`: the model relations that drive the pruning now follow every generated class a property type can reach, so the normalizers of a whitelisted operation stop targeting classes that were never generated (`NotNormalizableValueException: … no supporting normalizer found` on a valid response). `ConstraintReferencesTest` is retired: the Mago gate reports every dangling class reference ([#1066](https://github.com/janephp/janephp/issues/1066), `non-existent-class-like`)
- [OpenApi3] [OpenApi31] `whitelisted-paths` no longer prunes the request body model of a `multipart/form-data` operation: the relations of a whitelisted operation followed `application/json`, `*+json` and `application/x-www-form-urlencoded` bodies only, so an upload endpoint degraded to `\stdClass $requestBody` and its body could not be normalized by the generated `JaneObjectNormalizer` (`NotNormalizableValueException` on every upload unless a Symfony `ObjectNormalizer` was registered as an additional normalizer)
...
- [OpenApi31] `multipart/form-data` file parts declared the OpenAPI 3.1 way are recognized again. OpenAPI 3.1 (JSON Schema 2020-12) dropped `format: binary` and spells a binary payload `type: string` + `contentMediaType: <media type>`, but the multipart generator only detected the 3.0 spelling: such a property produced no `$partOptions` entry at all, so the part went on the wire without a `filename` in its `Content-Disposition` header — the filename-less part servers like FastAPI or Spring reject as "not a file" ([GH#1036](https://github.com/janephp/janephp/issues/1036)). A `type: string` property carrying a `contentMediaType` now gets the property-name `filename` fallback exactly like a `format: binary` one, and composes with the per-part `Content-Type` emitted from the media type's `encoding` object (that path was already independent of the binary detection and is unchanged). A `contentEncoding` is deliberately not required: JSON Schema 2020-12 defines `contentMediaType` without it as the string already being the raw content of that media type. Specifications using `format: binary` — including 3.1 documents converted from 3.0 — generate byte-identical code, and the guessed model property type is untouched
...
- [OpenApi2] Generating from a Swagger 2.0 specification whose response schema declares neither `type` nor `enum` — or a `number` without a `format` and without bounds — no longer fatals with `Typed property Jane\Component\OpenApi2\JsonSchema\Model\Schema::$enum must not be accessed before initialization`. Deriving the scalar return type of an endpoint now reads `type`, `enum`, `format`, `default`, `minimum`, `maximum` and `multipleOf` through the same `?? null` guard as the OpenAPI 3.x counterpart: the schema model only assigns the keywords the document declares, so every other typed property stays uninitialized (surfaced by the corpus smoke job of [#1080](https://github.com/janephp/janephp/issues/1080) on the public Slack Web API 2.0 specification)
