...
- [OpenApi] Generated endpoints returning a raw `json_decode()` of the response body no longer silently return `null` on malformed JSON: the decode now uses `JSON_THROW_ON_ERROR` and a `\JsonException` is converted into a `RuntimeException('Malformed JSON response body.')` chained to the original exception, consistently for OpenAPI 2 / 3 / 3.1
