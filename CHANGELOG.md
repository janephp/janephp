...
## [Unreleased]
### Added
- [JsonSchema] [OpenApi] Console `generate` commands now print generation progress: a header per schema (`Generating for schema …` / `Output: …`), a `✔ Guessing` / `✔ Generating` phase line, the grouped file counts (models, enums, normalizers, validators, runtime, auth, client, endpoints, exceptions) with the `+2` normalizer detail when `reference` is on, and `Done in Xs`. Silent at `--quiet` ([#252](https://github.com/janephp/janephp/issues/252), [ADR 0013](docs/contributing/adrs/0013-generation-events.md))
- [JsonSchema] [OpenApi] New **experimental** generation events system ([#859](https://github.com/janephp/janephp/issues/859), [ADR 0013](docs/contributing/adrs/0013-generation-events.md)): a dependency-free `Jane\Component\JsonSchema\Event\EventDispatcher` with symfony-style subscribers (`GenerationSubscriberInterface::getSubscribedEvents()`). Lifecycle events (`GenerationStarted/EndedEvent`, `SchemaStarted/EndedEvent`, `GuessingStarted/EndedEvent`, `GeneratingStarted/EndedEvent`, `FileGeneratedEvent`) carry readonly payloads; mutation events carry **live mutable references**: `PropertyGuessedEvent` lets listeners replace a guessed property type (`Property::setType()`) so models and normalizers stay consistent, and `PropertyGeneratedEvent` / `ClassGeneratedEvent` expose the model's PhpParser AST (`Stmt\Property` / `Stmt\Class_`) for docblock types, psalm tags, custom methods, traits or extends. Event constructors are frozen; the event set may grow. Throwing listeners abort generation through the existing `GenerationFailedException` wrapping
### Removed
- [OpenApi] New `x-fetch-mode` OpenAPI extension on **GET and HEAD** operations (values `lazy` — the default, `eager`, `preload`), plus the `default-fetch-mode` generation option (mirrors `default-additional-properties` plumbing, GET/HEAD only). `lazy` returns a `Result` deferring the send to first access; `eager` is the historical blocking behavior; `preload` registers the request immediately and drives all in-flight requests of the client concurrently via `Client::stream()` / `Result::await()`. Every other verb is always eager, and misuses (`x-fetch-mode` on a non-GET/HEAD operation, invalid values) fail generation with clean, complete `[ERROR]` blocks through a new `FetchModeValidator` plugged into each component's `SchemaParser::validateSchema()` ([ADR 0002](docs/contributing/adrs/0002-pre-generation-schema-validation.md), [ADR 0012](docs/contributing/adrs/0012-symfony-httpclient-migration-x-fetch-mode.md))
- [OpenApi] New `Jane\Component\OpenApiRuntime\Client\Result` wrapper (`getStatusCode()`, `toObject()`, `toArray()`, `isInitialized()`, `await()`, `cancel()`) parsing on access through the endpoint's own status-code mapping, plus `Client::stream(iterable<Result|ResponseInterface>)`. Garbage-collecting an unconsumed `Result` aborts its transfer (documented drop-to-cancel); cancelling a not-yet-sent lazy `Result` prevents the send
...
### Removed
- **BC-breaking** [JsonSchema] [OpenApi] `Jane::build()` and `JaneOpenApi::build()` (and their JsonSchema/OpenApi2/OpenApi3/OpenApi31 subclasses) now take a **required second argument**: the `Jane\Component\JsonSchema\Event\EventDispatcher` used for generation progress and customization hooks (see the Added entry). Programmatic callers pass `new EventDispatcher()` when no listener is needed; the console always provides one ([ADR 0013](docs/contributing/adrs/0013-generation-events.md))
